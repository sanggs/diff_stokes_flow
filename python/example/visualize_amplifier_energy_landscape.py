...
if __name__ == '__main__':
    ...
    for demo_name in demo_names:
        ...
        for j in range(1):
            ...
            info_dict = {}
            x2 = env.sample()
            noise = np.array(x2) - np.array(x)
            info_dict["noise"] = np.random.rand(np.shape(np.array(x))[0]) * 0.25
            ...
            with open('bz_ns_energy_vals_{}_{}.txt'.format(j, num_pieces_per_curve[0]), 'w') as f:
                for entry in evals:
                    f.write("{} {}\n".format(entry[0], entry[1]))

        env._render_vel_2d(fs, 'viz_bs_ns_linesearch_sample')
