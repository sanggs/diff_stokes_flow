...
all_demo_names = {
    ...
    'piecewise_controls': ('piecewise_controls_env_2d', 'PiecewiseControls2d')
}

def get_best_init(env, sample_num, demo_name):
...
def load_best_sample(env, sample_num, demo_name):
    get_best_init(env, sample_num, demo_name)
    # Load from file.
    ...
    return best_sample, unit_loss

def run_optimization(env, best_sample, unit_loss, max_iter):
    ...
    lattice_bound = np.ones_like(lattice_init) * 5.0
    lattice_bound = np.reshape(lattice_bound, (2, -1, 2))

    lattice_bound[0][0][0] = 0.0
    lattice_bound[0][-1][:] = 0.0
    lattice_bound[1][0][:] = 0.0
    lattice_bound[1][-1][0] = 0.0

    lattice_bound = np.reshape(lattice_bound, np.shape(lattice_init))
    ...
    global opt_history 
    opt_history = [(lattice_init.copy(), loss, grad.copy())]
    ...
    start_config = lattice_init.copy()
    for iter in range(max_iter):
        results = scipy.optimize.minimize(loss_and_grad, start_config.copy(), method='L-BFGS-B', jac=True, bounds=bounds,
        ...
        print('Data saved to {}/{:04d}.data.'.format(demo_name, len(opt_history) - 1))
        start_config = np.copy(opt_history[-1][0])
        print_warning('Norm of start config = {}'.format(np.linalg.norm(start_config)))
...
def visualize_results(env, demo_name):
    ...
    for k in range(cnt):
        ...
        x = xk
        env.render(x, '{:04d}.png'.format(k), { 'solver': solver, 'spp': spp })
...
if __name__ == '__main__':
    ...
    print_info("Loaded environment")

    (cx, cy) = env._cell_nums
    unique_id = '_{}_{}'.format(cx, cy)
    ...
    best_sample, unit_loss = load_best_sample(env, sample_num, demo_name)

    run_optimization(env, best_sample, unit_loss, max_iter)

    # Load results from demo_name.
    visualize_results(env, demo_name)
